...
bool Adafruit_ADXL343::begin(uint8_t i2caddr) {
  ...
  writeRegister(ADXL343_REG_DUR, 300);        // Max tap duration, 625 µs/LSB
  writeRegister(ADXL343_REG_LATENT, 100); // Tap latency, 1.25 ms/LSB, 0=no double tap
  writeRegister(ADXL343_REG_WINDOW, 400); // Waiting period,  1.25 ms/LSB, 0=no double tap
